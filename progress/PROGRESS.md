# My Progress

Track your progress through the curriculum here. Update this as you complete notebooks.

## Module 01 — Python Basics

- [ ] `01_variables_and_types.ipynb`
- [ ] `02_control_flow.ipynb`
- [ ] `03_functions.ipynb`
- [ ] `04_collections.ipynb`
- [ ] `05_review_and_practice.ipynb`

## Module 02 — Data Workflows

- [ ] `01_imports_and_modules.ipynb`
- [ ] `02_files_and_io.ipynb`
- [ ] `03_json_and_csv.ipynb`
- [ ] `04_review_and_practice.ipynb`

## Module 03 — Debugging

- [ ] `01_reading_errors.ipynb`
- [ ] `02_debugging_strategies.ipynb`
- [ ] `03_review_and_practice.ipynb`

## Module 04 — NumPy & Pandas

- [ ] `01_numpy_arrays.ipynb`
- [ ] `02_pandas_dataframes.ipynb`
- [ ] `03_groupby_and_aggregation.ipynb`
- [ ] `04_eval_data_mini_project.ipynb`

## Module 05 — Plotting

- [ ] `01_matplotlib_basics.ipynb`
- [ ] `02_distributions_and_scatter.ipynb`
- [ ] `03_heatmaps_and_subplots.ipynb`
- [ ] `04_eval_plots_mini_project.ipynb`

## Module 06 — Reusable Scripts

- [ ] `01_script_structure.ipynb`
- [ ] `02_argparse_and_logging.ipynb`
- [ ] `03_eval_script_mini_project.ipynb`

## Module 07 — Environments & Reproducibility

- [ ] `01_virtual_environments.ipynb`
- [ ] `02_package_management.ipynb`
- [ ] `03_seeds_and_reproducibility.ipynb`
- [ ] `04_mini_project.ipynb`

## Module 08 — Statistics for Experiments

- [ ] `01_descriptive_stats.ipynb`
- [ ] `02_distributions_and_spread.ipynb`
- [ ] `03_comparing_groups.ipynb`
- [ ] `04_mini_project.ipynb`

## Module 09 — Evaluation Methodology

- [ ] `01_eval_design.ipynb`
- [ ] `02_metrics_and_baselines.ipynb`
- [ ] `03_inter_rater_agreement.ipynb`
- [ ] `04_mini_project.ipynb`

## Module 10 — Model Output Analysis

- [ ] *(scaffold — expand when ready)*

## Module 11 — Prompt/Output Datasets

- [ ] *(scaffold — expand when ready)*

## Module 12 — Safety-Oriented Evaluation

- [ ] *(scaffold — expand when ready)*

## Module 13 — Error Analysis

- [ ] *(scaffold — expand when ready)*

## Module 14 — Research Coding Habits

- [ ] *(scaffold — expand when ready)*

## Module 15 — Research Memos

- [ ] *(scaffold — expand when ready)*

---

## Notes

*Use this space for personal observations, things that confused you, or insights to revisit.*

